"""M1.5 test: pyramidkv_update kernel correctness.

Each (frame_in_block, head) -> pool slot write is verified by populating
the source new_k/new_v with deterministic data, calling pyramidkv_update,
and checking the manager's pool slots match the corresponding source
chunks.

Companion to test_pyramidkv_pack: pack reads pools, update writes them.
...
class TestPyramidKVUpdate:
    ...
    def test_update_then_plan_then_pack_roundtrip(self):
        ...
        cu, sk, sg, sl, dst_off = torch.ops.adahead.pyramidkv_plan(mgr, 0, 1)
        torch.ops.adahead.pyramidkv_pack(mgr, sk, sg, sl, dst_off)
        torch.cuda.synchronize()
