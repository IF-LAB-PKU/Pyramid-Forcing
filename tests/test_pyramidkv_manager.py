"""M1.3 test: PyramidKVCacheManager allocation + reset + set_strategy.
...
3. set_strategy() writes the kind+params slot for one (layer, head) without
   touching others.
4. The custom class is reachable via torch.classes.adahead.PyramidKVCacheManager
...
@pytest.mark.gpu
class TestPyramidKVCacheManager:
