"""Shadow validator: mirror Python cache state into the C++ HeadKVCacheManager
and check that ``torch.ops.adahead.headkv_plan + headkv_pack`` produces the
same V as Python ``HeadKVCache.get_flat_kv()``.

Designed as a side-by-side validator with zero impact on the main code path
when ``HEADKV_USE_CPP_PACK`` is unset/0. Enable only for debugging the C++
...
def cpp_pack_enabled() -> bool:
    return os.environ.get("HEADKV_USE_CPP_PACK", "0") == "1" or cpp_pack_output_enabled()
...
def cpp_pack_output_enabled() -> bool:
    ...
    **WARNING — experimental, not production-ready.** Enabling this flag
    ...
    """
    return os.environ.get("HEADKV_USE_CPP_PACK_OUTPUT", "0") == "1"
...
def maybe_attach_shadow(
    *,
    ...
    from headkv._ops import _ensure_loaded
    if not _ensure_loaded():
        import warnings
        warnings.warn("[HEADKV_SHADOW] extension load failed — skipping shadow")
        return None
    return _ShadowState(
        layer_idx=layer_idx,
        num_heads=num_heads,
...
class _ShadowState:
    """Holds a per-layer HeadKVCacheManager and mirrors Python cache state into
    its pools so `torch.ops.adahead.headkv_plan + headkv_pack` can be run as a
    parallel validator."""
    ...
    def cpp_pack_v(self) -> torch.Tensor:
        ...
        cu, sk, sg, sl, dst = ns.headkv_plan(self.mgr, 0, 0)
        _ops_mod.headkv_pack(self.mgr, sk, sg, sl, dst)
    ...
    def assert_v_matches(self, python_v_flat: torch.Tensor) -> None:
        ...
        not a frame_seqlen multiple (headkv/merge.py:182). These get
        ...
        if diff > 0.0:
            ...
            if self._mismatch_count == 1:
                ...
                import os
                if os.environ.get("HEADKV_SHADOW_DEBUG", "0") == "1":
    ...
    def _dump_first_mismatch(self, cpp_v: torch.Tensor, py_v: torch.Tensor) -> None:
        ...
        print(f"[HEADKV_SHADOW_DEBUG] layer {self.layer_idx} first mismatch:", file=sys.stderr)
