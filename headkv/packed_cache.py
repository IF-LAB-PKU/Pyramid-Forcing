"""HeadKVPackedCache: minimal Python wrapper around the C++ plan/pack/update ops.
...
``max_recent_frames``. It exists to validate that ``headkv_plan``,
``headkv_pack`` and ``headkv_update`` interoperate on realistic-shape data
...
class HeadKVPackedCache:
    """Minimal cache backed by torch.classes.adahead.HeadKVCacheManager.
    ...
    def readout(self, current_t: int = 0, pass_kind: int = 0) -> _PackedReadout:
        ...
        cu, sk, sg, sl, dst = torch.ops.adahead.headkv_plan(
            self._mgr, int(current_t), int(pass_kind)
        )
        _ops_mod.headkv_pack(self._mgr, sk, sg, sl, dst)
