...
When ``HEADKV_PARITY_DUMP_DIR`` is set, every call to
...
to ``$HEADKV_PARITY_DUMP_DIR/$HEADKV_PARITY_DUMP_KIND/call_NNNNNN.pt``.

Env vars:
  HEADKV_PARITY_DUMP_DIR   — base output dir (required to enable)
  HEADKV_PARITY_DUMP_KIND  — subdir (e.g. "adaptive" or "mega"; default "unknown")
  HEADKV_PARITY_MAX_CALLS  — stop dumping after N calls (default unlimited).
                             For 24f, one prompt = 960 calls (4 steps × 8 blocks × 30 layers).

Usage:
  # AdaptiveKVCache baseline (24f, one prompt):
  HEADKV_PARITY_DUMP_DIR=videos/parity \\
  HEADKV_PARITY_DUMP_KIND=adaptive \\
  HEADKV_PARITY_MAX_CALLS=960 \\
  python inference.py ... --data_path prompts/parity_one.txt --num_output_frames 24
  # MegaCache run (same seed/config):
  HEADKV_PARITY_DUMP_DIR=videos/parity \\
  HEADKV_PARITY_DUMP_KIND=mega \\
  HEADKV_PARITY_MAX_CALLS=960 \\
  HEADKV_USE_MEGA_CACHE=1 \\
  python inference.py ... --data_path prompts/parity_one.txt --num_output_frames 24
  # Run twice (once per mega-cache toggle), diff the resulting dump dirs
  # manually to confirm bit-exact parity.

When ``HEADKV_PARITY_DUMP_DIR`` is unset, all functions are O(1) no-ops.
"""
...
def _init() -> None:
    ...
    base = os.environ.get("HEADKV_PARITY_DUMP_DIR")
    if not base:
        return
    kind = os.environ.get("HEADKV_PARITY_DUMP_KIND", "unknown")
    out = Path(base) / kind
    ...
    max_calls_env = os.environ.get("HEADKV_PARITY_MAX_CALLS")
    if max_calls_env:
        try:
    ...
    with _lock:
        ...
        if max_calls is not None and counter >= max_calls:
            if not _state["max_reached_announced"]:
                ...
                sys.stderr.write(
                    f"[parity_dump] reached HEADKV_PARITY_MAX_CALLS={max_calls}; "
