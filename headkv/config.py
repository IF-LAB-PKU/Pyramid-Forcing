...
# Tracks config_paths already announced via the "Loading HeadKV config" line.
# MegaCache rebuilds HeadKVConfig on every prompt (the manager is wiped per
...
class HeadKVConfig:
    """Central configuration for HeadKV caches across a whole model.
    ...
    decide their behavior:
      ...
      * **compositions** — populated externally by
        ``headkv.factory.build_compositions``; the list-of-lists of
    ...
    individual ``HeadKVCache`` / ``AdaptiveKVCache`` instances read at
    construction time.
