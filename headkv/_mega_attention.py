...
def mega_attention_step(
    ...
    *,
    ...
    Pipeline:
      ...
      - Runs ``headkv_update`` to copy new K/V into the middle pool slots.
    ...
    # ---- Step 2: headkv_update — write K/V into middle pool slots ----
    # mega_state_update emits per-head local slot indices (0..max_middle-1
    # for middle). headkv_update expects slot_global = (l*H+h)*max_slots+slot.
    max_middle = int(mgr.max_middle())
...
def _layer_offset_slot_global(
    ...
    [L, H, ...] pool layout that headkv_pack expects.
