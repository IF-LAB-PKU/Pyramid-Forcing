...
//      (existing headkv_update_kernel) to consume
//
// The data movement itself stays in headkv_update_kernel — this kernel only
...
// Kind encoding matches existing headkv_update_kernel:
...
// Mirrors headkv/_mega_state_ref.py:_update_merge.
...
// The descriptor format matches the existing headkv_update_kernel contract so
