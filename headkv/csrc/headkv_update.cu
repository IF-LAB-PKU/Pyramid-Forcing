// headkv_update_kernel: write incoming K/V frames into pool slots.
//
// Companion to headkv_pack: pack READS pool slots into a flat workspace,
...
__global__ void headkv_update_kernel(
...
void launch_headkv_update(
    ...
    TORCH_CHECK(new_k.is_cuda() && new_v.is_cuda(),
                "headkv_update: new_k/new_v must be on CUDA");
    TORCH_CHECK(new_k.scalar_type() == at::kBFloat16,
                "headkv_update currently only implements bf16");
    ...
    if (new_pos.defined() && new_pos.numel() > 0) {
        TORCH_CHECK(new_pos.dtype() == at::kLong,
                    "headkv_update: new_pos must be int64");
    ...
    headkv_update_kernel<<<N, kThreads, 0, stream>>>(
