// HeadKVCacheManager implementation.
//
...
at::ScalarType parse_kv_dtype(const std::string& s) {
    ...
    TORCH_CHECK(false, "HeadKVCacheManager: unsupported kv dtype '", s, "'");
}
...
HeadKVCacheManager::HeadKVCacheManager(
    int64_t num_layers,
    int64_t num_heads,
    int64_t head_dim,
    ...
    TORCH_CHECK(num_layers > 0 && num_heads > 0 && head_dim > 0,
                "HeadKVCacheManager: num_layers/num_heads/head_dim must be > 0");
    TORCH_CHECK(frame_seqlen > 0,
                "HeadKVCacheManager: frame_seqlen must be > 0");
    TORCH_CHECK(max_sink_frames >= 0 && max_middle_frames >= 0 && max_recent_frames >= 0,
                "HeadKVCacheManager: max_*_frames must be >= 0");
    TORCH_CHECK(max_attend_chunks > 0,
                "HeadKVCacheManager: max_attend_chunks must be > 0 "
...
int64_t HeadKVCacheManager::max_pack_tokens() const {
...
int64_t HeadKVCacheManager::max_merge_blocks() const {
    return static_cast<int64_t>(kMaxMergeBlocks);
}
int64_t HeadKVCacheManager::max_merge_active() const {
    return static_cast<int64_t>(kMaxMergeActive);
}
int64_t HeadKVCacheManager::max_merge_tokens_per_anchor() const {
    return static_cast<int64_t>(kMaxMergeTokens);
}

void HeadKVCacheManager::reset() {
    write_idx_.zero_();
    valid_count_.zero_();
...
void HeadKVCacheManager::set_strategy(
    int64_t layer_idx,
    int64_t head_idx,
    int64_t kind,
    torch::Tensor params
    ...
    TORCH_CHECK(layer_idx >= 0 && layer_idx < num_layers_,
                "HeadKVCacheManager::set_strategy layer_idx out of range");
    TORCH_CHECK(head_idx >= 0 && head_idx < num_heads_,
                "HeadKVCacheManager::set_strategy head_idx out of range");
    TORCH_CHECK(params.dtype() == at::kChar,
                "HeadKVCacheManager::set_strategy params must be int8");
