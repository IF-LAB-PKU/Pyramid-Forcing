...
//
// Successor to HeadKVPlanInfo (M1.4) with one additional output:
//   anchor_t_remap : per-frame remapped t for dynamic RoPE.
//
// The existing pack kernel (headkv_pack) only uses src_kind / src_slot_global
// / seg_lengths / dst_token_offsets / cu_seqlens_k. Day 1 emits these
// identically to headkv_plan so M3 can drop in mega_plan as a strict
