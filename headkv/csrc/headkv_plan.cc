// headkv_plan: CPU-side layout planning.
//
// Reads manager state (valid_count, head_t_table) and produces the
// descriptor tensors consumed by headkv_pack. One H2D copy at the end.
//
...
> headkv_plan(
    ...
    TORCH_CHECK(mgr.get() != nullptr, "headkv_plan: manager is null");
    (void)current_t;
    (void)pass_kind;
...
> _headkv_plan_cuda(
    ...
    int64_t current_t,
    int64_t pass_kind
) { return headkv_plan(mgr, current_t, pass_kind); }
...
> _headkv_plan_meta(
    ...
    int64_t current_t,
    int64_t pass_kind
) { return headkv_plan_meta(mgr, current_t, pass_kind); }
