...
//
// Successor to headkv_plan (M1.4). Differences:
...
//     identity remap; Day 2 ports map_dynamic_pos_time from headkv/rope.py.
//
// Pack-kernel contract: each segment is one frame (seg_length = frame_seqlen
// in tokens). The existing headkv_pack kernel iterates segments in parallel
// and does plain memcpy, so per-frame granularity is a strict generalization
// of the M1.4 per-kind-per-head segments.
//
// Inputs (per-layer):
//   mgr               : HeadKVCacheManager (for max dims + valid_count[layer])
...
// Mirrors headkv/rope.py:map_sink_time. Sink frames sit in absolute-t domain
...
// Mirrors headkv/rope.py:map_dynamic_pos_time. Applies to non-sink anchors
