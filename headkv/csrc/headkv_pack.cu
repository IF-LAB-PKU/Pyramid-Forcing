// headkv_pack_kernel: single mega-kernel that gathers K/V from
...
// has been built by headkv_plan() (CPU-side layout, one H2D copy of
// int32 descriptors).
//
...
__global__ void headkv_pack_kernel(
...
void launch_headkv_pack(
    ...
    TORCH_CHECK(sink_pool.scalar_type() == at::kBFloat16,
                "headkv_pack currently only implements bf16 pools");
    ...
    headkv_pack_kernel<<<N, kThreads, 0, stream>>>(
