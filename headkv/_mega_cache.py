...
mega path is enabled (env ``HEADKV_USE_MEGA_ATTN=1``).

Design:
  - One ``HeadKVCacheManager`` covers all L layers' pools (shared allocator).
...
Pipeline integration:
  ...
  - ``CausalWanSelfAttention.forward``: if ``kv_cache`` is a ``MegaCache``,
    dispatch to ``mega_attention_step`` instead of ``headkv_attention``.
"""
...
class MegaCacheCtx:
    ...
    mgr: object                # torch.classes.adahead.HeadKVCacheManager
...
class MegaCache:
    ...
    a HeadKV cache instance (layer_idx, frame_seq_length, ...). All state
    ...
    Public methods:
      ``update(new_k, new_v, current_start)`` — insert new frames into
          ...
          and middle insertion (C++ mega_state_update + headkv_update).
    ...
    def update(
        ...
        *,
        ...
        # ---- Step 3: middle via mega_state_update + headkv_update ----
        ...
        if cache_update_mode == "clean":
            ...
            # Split descriptors: kinds 0/1/2 go to headkv_update (writes to
            ...
            # Thread per-token pos into headkv_update so the kernel writes
    ...
    # ------------------------------------------------------------------
    # Readout path: mega_plan + headkv_pack + flash_attn_varlen
    # ------------------------------------------------------------------
    def attend(
        self,
        ...
        *,
        ...
        Pipeline (one kernel/op launch each):
          ...
          2. ``headkv_pack`` — writes all chunks' K/V/pos to disjoint regions of
...
def build_mega_caches(
    ...
    *,
    ...
    Args:
        ...
        max_*_frames: pool capacities, fed to HeadKVCacheManager constructor.
        sink/history kwargs: dynamic-RoPE remap config; defaults match
            pyramid_forcing10.

    Returns ``num_layers`` MegaCache instances, all sharing one
    HeadKVCacheManager and one [L, H * sizeof(PerHeadState)] state tensor.
    """
