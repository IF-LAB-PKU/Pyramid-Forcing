...
``headkv/_mega_state_ref.py`` into a raw byte buffer with the **exact same
ABI** the C++ compiler used. We do this with a ``numpy`` structured dtype
mirroring the field layout in ``headkv/csrc/anchor_store.cuh``.
...
# Mirror of headkv/csrc/anchor_store.cuh:PerHeadState. Field order MUST match
...
def _check_abi() -> None:
    ...
    if cpp_size != py_size:
        raise RuntimeError(
            ...
            f"headkv/csrc/anchor_store.cuh and the dtype in "
            f"headkv/_mega_state_ops.py must be kept in sync."
        )
    _ABI_CHECKED = True
...
def mega_plan_cuda(
    mgr,                                  # torch.classes.adahead.HeadKVCacheManager
    ...
    full map_sink_time / map_dynamic_pos_time remap from headkv/rope.py.
...
def mega_plan_multi_cuda(
    ...
    headkv_pack can write all chunks' K/V/pos into disjoint regions of
...
def mega_merge_accum_cuda(
    ...
    Args:
        mgr: HeadKVCacheManager (provides merge accumulator + pool views).
