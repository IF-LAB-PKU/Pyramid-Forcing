...
Mirrors the C++ logic in headkv/csrc/anchor_store_update.cu byte-for-byte so
we can:
...
Mirrors the field layout in headkv/csrc/anchor_store.cuh:PerHeadState. Field
...
# Destination kind encoding — must match scatter_copy_bind.cpp / headkv_update.cu
# 0/1/2/-1 are pre-existing for sink/middle/recent/skip — used by the existing
# headkv_update_kernel data-movement path. The MERGE_ACCUM kind is NEW: it
...
class MergeDuplicateError(ValueError):
    ...
    Mirrors the ValueError at headkv/merge.py:108 so callers can pin down
...
def _update_merge(
    ...
    Mirrors headkv/merge.py:65 update() per-frame logic:
