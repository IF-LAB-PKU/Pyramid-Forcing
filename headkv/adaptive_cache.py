...
class AdaptiveKVCache(HeadKVCache):
    ...
    Extends :class:`HeadKVCache` with the full Pyramid Forcing policy stack:
      ...
      * **Workspace-pinned packing** — flat K/V/pos are written into
        ...
        state into ``HeadKVCacheManager`` for kernel parity testing.
    ...
    :class:`HeadKVConfig`; the rest of the constructor args toggle
    ...
    def __init__(
        ...
        # Optional C++ shadow validator gated by HEADKV_USE_CPP_PACK=1. Lazy-
    ...
    def _ensure_shadow(self) -> None:
        ...
        No-op when HEADKV_USE_CPP_PACK is unset or when batch_size != 1
    ...
    def _shadow_swap_v(self, v_flat_slice: torch.Tensor, spec) -> None:
        """When HEADKV_USE_CPP_PACK_OUTPUT=1, replace v_flat with C++ pack V.
        ...
        if self._shadow is None:
            return
        from headkv._cpp_shadow import cpp_pack_output_enabled
    ...
    def _shadow_assert_v(self, v_flat_slice: torch.Tensor, spec=None) -> None:
        ...
        try:
            ...
            if self._shadow.mismatch_count == 1:
                import warnings
                warnings.warn(f"[HEADKV_SHADOW] V mismatch on layer {self.layer_idx}: {exc}")
    ...
    def _try_write_through_readout_tail(
        ...
        *,
        ...
        if os.environ.get("HEADKV_WRITE_THROUGH_READOUT") != "1":
            return False
    ...
    def _try_fast_path_noisy_overwrite(
        ...
        1. HEADKV_DISABLE_M6_FASTPATH=1 not set
        ...
        """
        if os.environ.get("HEADKV_DISABLE_M6_FASTPATH") == "1":
            return False
    ...
    def _refresh_rope_only_for_cs(
        ...
        *,
        ...
        # Path A+B v2 (HEADKV_PATH_AB_V2): preserve _readout_cache_current_start
        ...
        if os.environ.get("HEADKV_PATH_AB_V2", "1") != "0":
    ...
    def _cuda_refresh_opt_in(self) -> bool:
        return os.environ.get("HEADKV_CUDA_REFRESH", "0").strip().lower() in ("1", "true", "yes", "on")
    ...
    def _refresh_cached_readout_mutable_segments(
        ...
        *,
        ...
        if (
            ...
            and os.environ.get("HEADKV_DISABLE_M6_FASTPATH") != "1"
        ):
        ...
        *,
        ...
        if spec.total_len > 0:
            if spec.segments:
                ...
                if os.environ.get("HEADKV_PTR_DEBUG"):
    ...
    def get_decoupled_flat_kv_and_frames(
        ...
        # HEADKV_PATH_AB=0 disables (for A/B benchmarking).
        if os.environ.get("HEADKV_PATH_AB", "1") != "0" and self._can_reuse_readout_cache_for_rope_only(
