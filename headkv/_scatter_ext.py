...
def _try_load():
    ...
    if not torch.cuda.is_available():
        _available = False
        return False
    verbose = os.environ.get("HEADKV_VERBOSE_BUILD") == "1"
    try:
        _set_cuda_arch_list_if_unset()
        from torch.utils.cpp_extension import load
        _dir = os.path.dirname(os.path.abspath(__file__))
        _module = load(
            name="headkv_scatter_v30",  # bumped: Plan B — manager takes max_attend_chunks; k_flat_out / v_flat_out / pos_flat_out shrunk to per-attend-call capacity
            sources=[
                os.path.join(_dir, "csrc", "scatter_copy_bind.cpp"),
                os.path.join(_dir, "csrc", "scatter_copy.cu"),
                os.path.join(_dir, "csrc", "headkv_manager.cc"),
                os.path.join(_dir, "csrc", "headkv_pack.cu"),
                os.path.join(_dir, "csrc", "headkv_plan.cc"),
                os.path.join(_dir, "csrc", "headkv_update.cu"),
        ...
        if verbose:
            ...
            import sys
            print(
                f"[headkv] JIT build failed: {exc!r}. "
                "Set HEADKV_VERBOSE_BUILD=1 for full compile log.",
                file=sys.stderr,
            )
        _available = False
    return _available
...
def scatter_available() -> bool:
    ...
    torch.cat(out=workspace). Set HEADKV_FORCE_SCATTER=1 to enable.
    """
    if os.environ.get("HEADKV_FORCE_SCATTER") != "1":
        return False
    return _try_load()
...
def cuda_refresh_available() -> bool:
    ...
    HEADKV_CUDA_REFRESH opt-in and does not use HEADKV_FORCE_SCATTER.
    """
    return _try_load()
