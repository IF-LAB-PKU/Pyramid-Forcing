"""Opt-in contiguous anchor storage for HeadKV middle strategies."""
...
def contig_anchor_store_requested() -> bool:
    if os.environ.get("HEADKV_CPP_STRATEGY", "0").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
        ...
        return False
    return os.environ.get("HEADKV_CONTIG_ANCHOR_STORE", "0").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
